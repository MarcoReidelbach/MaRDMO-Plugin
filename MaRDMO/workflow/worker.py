...
class prepareWorkflow:
    ...
    def preview(self, data):
        ...
        if data.get('model',{}).get('ID'):
            ...
            basic = query_sparql(query, ENDPOINTS['mardi']['sparql'])
            if basic:
        ...
        if data.get('model', {}).get('task'):            
            ...
            variables = query_sparql(query, ENDPOINTS['mardi']['sparql'])
            if variables:
            ...
            parameters = query_sparql(query, ENDPOINTS['mardi']['sparql'])
            if parameters:
        ...
        for dataset in data.get('dataset', {}).values():
            ...
            if dataset.get('Size'):
                if dataset['Size'][0] == options['kilobyte']:
                    ...
                    object = {"amount":f"+{dataset['Size'][1]}","unit": f"{ENDPOINTS['mardi']['uri']}/entity/{self.items['kilobyte']}"}
                elif dataset['Size'][0] == options['megabyte']:
                    verb = self.properties['data size']
                    object = {"amount":f"+{dataset['Size'][1]}","unit": f"{ENDPOINTS['mardi']['uri']}/entity/{self.items['megabyte']}"}
                elif dataset['Size'][0] == options['gigabyte']:
                    verb = self.properties['data size']
                    object = {"amount":f"+{dataset['Size'][1]}","unit": f"{ENDPOINTS['mardi']['uri']}/entity/{self.items['gigabyte']}"}
                elif dataset['Size'][0] == options['terabyte']:
                    verb = self.properties['data size']
                    object = {"amount":f"+{dataset['Size'][1]}","unit": f"{ENDPOINTS['mardi']['uri']}/entity/{self.items['terabyte']}"}
